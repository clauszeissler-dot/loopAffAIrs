...
Volle Klartext+Technisch-Erklärungen liegen im Skill ai-code-security-audit; hier nur
ein kurzer Reminder zum sofortigen Gegensteuern während des Edits.
"""
import json
import re
import sys
...
# übernommen 2026-06-30, Reminder auf eine Zeile gekürzt.
PATTERNS = [
    {"name": "child_process_exec", "path_filter": lambda p: p.endswith(_JS_EXTS),
     "substrings": ["child_process.exec", "execSync("],
     "msg": "child_process.exec()/execSync() können Command Injection erlauben — execFile()/spawn() mit Argument-Array statt Shell-String nutzen."},
    {"name": "new_function_injection", "path_filter": lambda p: p.endswith(_JS_EXTS),
     "substrings": ["new Function"],
     "msg": "new Function() mit String-Interpolation ist Code Injection — niemals ungeprüften String in den Function-Body interpolieren."},
    {"name": "eval_injection", "regex": r"(?<![a-zA-Z0-9_\.])eval\(",
     "msg": "eval() führt beliebigen Code aus — JSON.parse()/ast.literal_eval()/sicherer Expression-Parser statt eval()."},
    ...
    {"name": "innerHTML_xss", "path_filter": lambda p: p.endswith(_JS_EXTS),
     ...
     "msg": "innerHTML mit ungeprüftem Inhalt ist ein XSS-Sink — textContent oder DOMPurify nutzen."},
    ...
    {"name": "document_write_xss", "path_filter": lambda p: p.endswith(_JS_EXTS),
     ...
     "msg": "document.write() ist XSS-anfällig — createElement()/appendChild() nutzen."},
    {"name": "pickle_deserialization", "path_filter": lambda p: p.endswith(_PY_EXTS),
     ...
     "msg": "pickle.load(s)/Unpickler aus ungeprüften Quellen erlaubt Remote Code Execution — JSON oder schema-validierte Deserialisierung (pydantic/msgspec) nutzen."},
    ...
    {"name": "shelve_open", "regex": r"\bshelve\.open\s*\(",
     "msg": "shelve.open() nutzt pickle intern — unsicher bei ungeprüften Quellen."},
    ...
    {"name": "pickle_wrapper_load",
     ...
     "msg": "joblib.load/pd.read_pickle/np.load(allow_pickle=True) entpacken intern pickle — unsicher bei ungeprüften Quellen."},
    ...
    {"name": "python_subprocess_shell",
     ...
     "msg": "subprocess mit shell=True erlaubt Command Injection — Argumente als Liste ohne shell=True übergeben."},
    {"name": "go_exec_shell_injection", "regex": r'exec\.Command\(\s*"(?:sh|bash|/bin/sh|/bin/bash)"',
     "msg": "exec.Command mit sh/bash erlaubt Command Injection — Argumente direkt ohne Shell übergeben."},
    {"name": "unsafe_yaml_load", "regex": r"\byaml\.load\s*\((?![^)\n]{0,80}\bSafe)",
     "msg": "yaml.load() ohne SafeLoader führt beliebigen Python-Code aus — yaml.safe_load() nutzen."},
    ...
    {"name": "torch_unsafe_load",
     ...
     "msg": "torch.load() ohne weights_only=True entpackt intern pickle — RCE-Risiko bei ungeprüften Dateien."},
    ...
    {"name": "tls_verification_disabled",
     ...
     "msg": "TLS-Verifikation deaktiviert — ermöglicht MITM-Angriffe."},
    {"name": "xml_unsafe_parse",
     ...
     "msg": "Python-Standard-XML-Parser sind anfällig für XXE/Billion-Laughs — defusedxml.ElementTree nutzen."},
    {"name": "script_src_without_sri",
     "regex": r'<script\s+(?![^>]{0,400}integrity\s*=)[^>]{0,200}src\s*=\s*["\'](?:https?:)?//[^"\']{1,300}["\'][^>]{0,100}>',
     "msg": "Externes <script> ohne integrity=\"sha384-...\" — Subresource Integrity gegen CDN-Kompromittierung ergänzen."},
    {"name": "github_actions_workflow",
     "path_check": lambda p: ".github/workflows/" in p and (p.endswith(".yml") or p.endswith(".yaml")),
     "msg": "GitHub-Actions-Workflow: ungeprüften Issue-/PR-Input (z.B. github.event.issue.title) nie direkt in run: interpolieren — über env: mit Quoting führen."},
]
